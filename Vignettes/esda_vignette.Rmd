...
```{r}
significance_map(guerry,"doncat",permutations = 99999,type = "joincount") +
  tm_borders() +
  tm_layout(legend.outside = TRUE,title = "Local Join Count Significance Map doncat")
```
...
For the corresponding significance map, we use `significance_map`, and use the same input variables.
```{r}
significance_map(guerry, c("doncat","infcat"),permutations = 99999,type = "joincount") +
  tm_borders() +
  tm_layout(legend.outside = TRUE,"Multivariate Local Join Count Significance Map")
```
